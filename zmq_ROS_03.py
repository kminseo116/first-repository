...
class DetectionNode(Node):
    def __init__(self):
        ...
        try:
            ...
            self.socket_detection.setsockopt_string(zmq.SUBSCRIBE, "")
    ...
    def listen_and_publish(self):
        ...
        while rclpy.ok():
            try:
                socks = dict(poller.poll(timeout=1000))  # 1초 타임아웃

                if self.socket_detection in socks:
                    message = self.socket_detection.recv_string()
                    msg = Empty()

                    if message == "button_detected":
                        self.button_publisher.publish(msg)
                        self.get_logger().info('ROS 발행: button_detected')
                    
                    elif message == "fire_detected":
                        self.fire_publisher.publish(msg)
                        self.get_logger().info('ROS 발행: fire_detected')
                    
                    elif message == "door_detected":
